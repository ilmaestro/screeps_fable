...
(*
type Globals =
    [<Global>] static member FIND_EXIT_TOP with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_EXIT_RIGHT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_EXIT_BOTTOM with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_EXIT_LEFT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_EXIT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_CREEPS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_MY_CREEPS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_HOSTILE_CREEPS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_SOURCES_ACTIVE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_SOURCES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_DROPPED_RESOURCES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_DROPPED_ENERGY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_STRUCTURES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_MY_STRUCTURES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_HOSTILE_STRUCTURES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_FLAGS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_CONSTRUCTION_SITES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_MY_CONSTRUCTION_SITES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_HOSTILE_CONSTRUCTION_SITES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_MY_SPAWNS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_HOSTILE_SPAWNS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member FIND_MINERALS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOP with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOP_RIGHT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RIGHT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member BOTTOM_RIGHT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member BOTTOM with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member BOTTOM_LEFT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LEFT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOP_LEFT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member OK with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_OWNER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NO_PATH with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NAME_EXISTS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_BUSY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_FOUND with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_ENOUGH_RESOURCES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_ENOUGH_ENERGY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_INVALID_TARGET with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_FULL with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_IN_RANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_INVALID_ARGS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_TIRED with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NO_BODYPART with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_NOT_ENOUGH_EXTENSIONS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_RCL_NOT_ENOUGH with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ERR_GCL_NOT_ENOUGH with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_RED with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_PURPLE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_BLUE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_CYAN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_GREEN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_YELLOW with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_ORANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_BROWN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_GREY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLOR_WHITE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member COLORS_ALL with get(): ResizeArray<float> = failwith "JS only" and set(v: ResizeArray<float>): unit = failwith "JS only"
    [<Global>] static member CREEP_SPAWN_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CREEP_LIFE_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CREEP_CLAIM_LIFE_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CREEP_CORPSE_RATE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member OBSTACLE_OBJECT_TYPES with get(): ResizeArray<string> = failwith "JS only" and set(v: ResizeArray<string>): unit = failwith "JS only"
    [<Global>] static member ENERGY_REGEN_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ENERGY_DECAY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    //[<Global>] static member CREEP_CORPSE_RATE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member REPAIR_COST with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RAMPART_DECAY_AMOUNT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RAMPART_DECAY_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RAMPART_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RAMPART_HITS_MAX with get(): RAMPART_HITS_MAXType = failwith "JS only" and set(v: RAMPART_HITS_MAXType): unit = failwith "JS only"
    [<Global>] static member SPAWN_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member SPAWN_ENERGY_START with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member SPAWN_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member SOURCE_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member SOURCE_ENERGY_NEUTRAL_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member SOURCE_ENERGY_KEEPER_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member WALL_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member WALL_HITS_MAX with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member EXTENSION_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member EXTENSION_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ROAD_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ROAD_WEAROUT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ROAD_DECAY_AMOUNT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ROAD_DECAY_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LINK_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LINK_HITS_MAX with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LINK_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LINK_COOLDOWN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LINK_LOSS_RATION with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member STORAGE_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member STORAGE_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member BODYPART_COST with get(): BODYPART_COSTType = failwith "JS only" and set(v: BODYPART_COSTType): unit = failwith "JS only"
    [<Global>] static member BODYPARTS_ALL with get(): ResizeArray<string> = failwith "JS only" and set(v: ResizeArray<string>): unit = failwith "JS only"
    [<Global>] static member CARRY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member HARVEST_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member HARVEST_MINERAL_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member REPAIR_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member DISMANTLE_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member BUILD_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member ATTACK_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member UPGRADE_CONTROLLER_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RANGED_ATTACK_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member HEAL_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member RANGED_HEAL_POWER with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member DISMANTLE_COST with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MOVE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member WORK with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member CARRY with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member ATTACK with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RANGED_ATTACK with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member TOUGH with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member HEAL with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member CLAIM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member CONSTRUCTION_COST with get(): CONSTRUCTION_COSTType = failwith "JS only" and set(v: CONSTRUCTION_COSTType): unit = failwith "JS only"
    [<Global>] static member CONSTRUCTION_COST_ROAD_SWAMP_RATIO with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_EXTENSION with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_RAMPART with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_ROAD with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_SPAWN with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_LINK with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_WALL with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_KEEPER_LAIR with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_CONTROLLER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_STORAGE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_TOWER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_OBSERVER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_POWER_BANK with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_POWER_SPAWN with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_EXTRACTOR with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_LAB with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_TERMINAL with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_CONTAINER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_NUKER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member STRUCTURE_PORTAL with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ENERGY with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_POWER with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_LEMERGIUM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_KEANIUM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_GHODIUM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_OXYGEN with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_HYDROGEN with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYST with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_HYDROXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM_KEANITE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM_LEMERGITE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM_HYDRIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM_OXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_KEANIUM_HYDRIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_KEANIUM_OXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_LEMERGIUM_HYDRIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_LEMERGIUM_OXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM_HYDRIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM_OXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_GHODIUM_HYDRIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_GHODIUM_OXIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_UTRIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_KEANIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_KEANIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_LEMERGIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_LEMERGIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_ZYNTHIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_GHODIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_GHODIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_UTRIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_UTRIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_KEANIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_KEANIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_LEMERGIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_LEMERGIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_ZYNTHIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_ZYNTHIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_GHODIUM_ACID with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCE_CATALYZED_GHODIUM_ALKALIDE with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member RESOURCES_ALL with get(): ResizeArray<string> = failwith "JS only" and set(v: ResizeArray<string>): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_LEVELS with get(): CONTROLLER_LEVELSType = failwith "JS only" and set(v: CONTROLLER_LEVELSType): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_STRUCTURES with get(): CONTROLLER_STRUCTURESType = failwith "JS only" and set(v: CONTROLLER_STRUCTURESType): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_DOWNGRADE with get(): CONTROLLER_DOWNGRADEType = failwith "JS only" and set(v: CONTROLLER_DOWNGRADEType): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_CLAIM_DOWNGRADE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_RESERVE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_RESERVE_MAX with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_MAX_UPGRADE_PER_TICK with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTROLLER_ATTACK_BLOCKED_UPGRADE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_ENERGY_COST with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_POWER_ATTACK with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_POWER_HEAL with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_POWER_REPAIR with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_OPTIMAL_RANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_FALLOFF_RANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TOWER_FALLOFF with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member OBSERVER_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member OBSERVER_RANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_CAPACITY_MAX with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_CAPACITY_MIN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_CAPACITY_CRIT with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_DECAY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_BANK_HIT_BACK with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_SPAWN_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_SPAWN_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_SPAWN_POWER_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member POWER_SPAWN_ENERGY_RATIO with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member EXTRACTOR_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_MINERAL_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_BOOST_ENERGY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_BOOST_MINERAL with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member LAB_COOLDOWN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member GCL_POW with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member GCL_MULTIPLY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member GCL_NOVICE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MODE_SIMULATION with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member MODE_SURVIVAL with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member MODE_WORLD with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member MODE_ARENA with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member TERRAIN_MASK_WALL with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERRAIN_MASK_SWAMP with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERRAIN_MASK_LAVA with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MAX_CONSTRUCTION_SITES with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MAX_CREEP_SIZE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MINERAL_REGEN_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member MINERAL_MIN_AMOUNT with get(): MINERAL_MIN_AMOUNTType = failwith "JS only" and set(v: MINERAL_MIN_AMOUNTType): unit = failwith "JS only"
    [<Global>] static member MINERAL_RANDOM_FACTOR with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERMINAL_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERMINAL_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERMINAL_SEND_COST with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member TERMINAL_MIN_SEND with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTAINER_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTAINER_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTAINER_DECAY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTAINER_DECAY_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member CONTAINER_DECAY_TIME_OWNED with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKER_HITS with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKER_COOLDOWN with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKER_ENERGY_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKER_GHODIUM_CAPACITY with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKE_LAND_TIME with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKE_RANGE with get(): float = failwith "JS only" and set(v: float): unit = failwith "JS only"
    [<Global>] static member NUKE_DAMAGE with get(): NUKE_DAMAGEType = failwith "JS only" and set(v: NUKE_DAMAGEType): unit = failwith "JS only"
    [<Global>] static member REACTIONS with get(): REACTIONSType = failwith "JS only" and set(v: REACTIONSType): unit = failwith "JS only"
    [<Global>] static member BOOSTS with get(): BOOSTSType = failwith "JS only" and set(v: BOOSTSType): unit = failwith "JS only"
    [<Global>] static member LOOK_CREEPS with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_ENERGY with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_RESOURCES with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_SOURCES with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_MINERALS with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_STRUCTURES with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_FLAGS with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_CONSTRUCTION_SITES with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_NUKES with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member LOOK_TERRAIN with get(): string = failwith "JS only" and set(v: string): unit = failwith "JS only"
    [<Global>] static member Memory with get(): Memory = failwith "JS only" and set(v: Memory): unit = failwith "JS only"
    [<Global>] static member Game with get(): Game = failwith "JS only" and set(v: Game): unit = failwith "JS only"
    [<Global>] static member PathFinder with get(): PathFinder = failwith "JS only" and set(v: PathFinder): unit = failwith "JS only"
*)
